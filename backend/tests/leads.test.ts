...
describe('GET /api/leads', () => {
  ...
  it('searches across name, email and phone', async () => {
    const byName = await request(app).get('/api/leads?search=grace');
    expect(byName.body.data[0].name).toBe('Grace Hopper');

    const byPhone = await request(app).get('/api/leads?search=7946');
...
describe('PATCH /api/leads/:id/status', () => {
  it('moves a lead to a new status', async () => {
    const created = await request(app).post('/api/leads').send(validLead);
    const res = await request(app)
      .patch(`/api/leads/${created.body.data.id}/status`)
...
describe('DELETE /api/leads/:id', () => {
  it('removes the lead', async () => {
    const created = await request(app).post('/api/leads').send(validLead);
    const res = await request(app).delete(`/api/leads/${created.body.data.id}`);
    expect(res.status).toBe(200);
  });
});
